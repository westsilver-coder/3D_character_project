...
### 카메라–이미지 대응 (형체가 안 나올 때)
Stage 2는 **synthetic orbit**만 사용하며, **카메라 i = 이미지 목록 i번째**로 1:1 짝을 짓는다. 실제 사진이 궤도 순서(예: 턴테이블 0°, 10°, 20°, …)가 아니면 "뷰 i 렌더"와 "이미지 i GT"가 맞지 않아 학습이 블러/물감 형태로 수렴한다. **해결**: (1) 입력 이미지를 궤도 각도 순서로 정렬해 두거나, (2) ROMP 등으로 이미지별 실제 R,t를 추정해 cameras.json을 채우는 방식으로 전환하는 것이 좋다.
